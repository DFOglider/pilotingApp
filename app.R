...
hfxlon <- c(-63.450000, -63.317000, -62.883000, -62.451000, -62.098000, -61.733000, -61.4333, -62.7527, -61.8326, -61.6167, -61.5167)
hfxlat <- c(44.400001, 44.267001, 43.883001, 43.479000, 43.183000, 42.850000, 42.475, 43.7635, 42.9402, 42.7333, 42.6183)
...
# modified bonavista line (BB1, BBn2, BBn3, BBn4, BBn1.5, BBn5)
bbmlon <- c(-52.967, -52.40, -51.83, -50.7, -52.76, -50.0)
bbmlat <- c(48.733, 49.065, 49.39, 50.00, 48.85, 50.83)
bbmname <- c('BB1', 'BBn2', 'BBn3', 'BBn4', 'BBn1.5', 'BBn5')
  ...
  observeEvent(input$download,{
    ...
    output$sciScaleBar <- renderUI({
      ...
      rng <- switch(input$SciVar,
                    ...
                    'Sal' = c(29, 35.5),
                    'salinityLegato' = c(29, 35.5),
                    'Conduc' = c(0,7),
                    'conductivityLegato' = c(0,7) * 10,
                    'SigTheta' = c(20, 28),
      ...
      value <- switch(input$SciVar,
                      ...
                      'salinityLegato' = unname(quantile(PLD$salinityLegato, probs = c(0.02, 0.98),  na.rm = TRUE)),
                      'Conduc' = unname(quantile(PLD$Conduc, probs = c(0.02, 0.98), na.rm = TRUE)),
                      'conductivityLegato' = unname(quantile(PLD$conductivityLegato, probs = c(0.02, 0.98),  na.rm = TRUE)),
                      'SigTheta' = unname(quantile(PLD$SigTheta, probs = c(0.02, 0.98), na.rm = TRUE)),
    ...
    if (input$Var == 'Navigation') {
      ...
      var <- input$SciVar
        data <- switch(var,
                       ...
                       'Sal' = PLD$Sal,
                       'salinityLegato' = PLD$salinityLegato,
                       'Conduc' = PLD$Conduc,
                       'conductivityLegato' = PLD$conductivityLegato,
                       'SigTheta' = PLD$SigTheta,
                       'SoundSpeed' = PLD$SoundSpeed,
        ...
        zlab <- switch(var,
                       ...
                       'temperatureLegato' = resizableLabel('T', axis = 'y'),
                       'Sal' = resizableLabel('S', axis = 'y'),
                       'salinityLegato' = resizableLabel('S', axis = 'y'),
                       'Conduc' = resizableLabel('conductivity S/m', axis = 'y'),
                       'conductivityLegato' = resizableLabel('conductivity mS/cm', axis = 'y'),
                       'SigTheta' = resizableLabel('sigmaTheta', axis = 'y'),
                       'SoundSpeed' = bquote('Sound speed ' * '[' * 'm/s' * ']'),
    ...
    map <- leaflet(as.data.frame(cbind(glon, glat)))%>%
      ...
      # 20210510 for some reason this doesn't work for me anymore ...
        ...
        addCircleMarkers(lng = bblon, lat = bblat,
          ...
          # bonavista modified
      addCircleMarkers(lng = bbmlon, lat = bbmlat,
                       radius = 7, fillOpacity = 0.5, stroke = F,
                       color = 'gray48',
                       popup = paste(sep = "<br/>",
                                     bbmname,
                                     paste0(as.character(round(bbmlat, 4)), ',', as.character(round(bbmlon, 3)))),
                       label = bbmname)%>%
